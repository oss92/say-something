...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddRecording'), wrapper.instance().toggleAddRecordingSection);
...
test('calling toggleAddRecordingSection dispatches toggleAddRecording', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddRecordingSection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddRecording()));
});
